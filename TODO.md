...
## 8. Design for variable layouts

## 9. Extra character handling
## 10. Producer page count control
The Producer has no mechanism to specify how many pages a chapter should contain. The LLM infers page count from synopsis length. Consider adding a `target_pages` or `target_panels` parameter to the Producer's `produce()` method. Low priority — does not block continuity work. Noted in DESIGN.md §15.
