...
16. [Scene Plan Format and Template System (Scope Redesign)](#16-scene-plan-format-and-template-system-scope-redesign)
...
**Exclusion-cue convention (roster context):** Character roster lines are built by the shared deterministic builder in `src/roster_context.py`, used identically by both producers. Two rules: (1) a `physical_description` field whose value contains the marker `rarely seen` describes a concealed feature and is NOT injected as a positive writing cue — the writing model never sees an eye or hair color it is forbidden to show (visible signature features carry no marker and pass through). (2) `prompt_tokens.exclusions` are promoted from a trailing footnote into per-rule `IDENTITY RULE` lines on the character's main roster entry, spliced verbatim with a fixed amplification template. The asset YAML and `physical_description` are unchanged — the vision Validation layer still reads the full description; only the Producer's writing context is redacted. The builder is pure code (no LLM calls): the same character YAML always yields byte-identical roster lines.
...
- **Seed source:** The `panel_seed` field in the PanelSpec — a hex byte (00–FF), e.g. `"A3"`, persisted in the PanelSpec. Scene path: deterministic — first hex byte of sha256(panel_id), so re-parsing the same scene is byte-identical. Chapter path (legacy): randomly generated at parse time. Both are stable across re-compilations of the same PanelSpec.
- **Decision:** Binary. Even hex byte value → inject; odd → do not inject. No size, orientation, or purpose variation in v1.
- **Injected string (verbatim when triggered):** `"This composition should include relatively unused area of low detail where a speech bubble will eventually be added. Don't draw the bubble, just compose the image so an editor can add it later."`
- **Regeneration behavior (legacy chapter path):** Re-parsing the same Chapter Plan generates new `panel_seed` values, producing different binary outcomes. Scene path: parse is a pure function — same scene, same seeds, same injection pattern.
...
## 16. Scene Plan Format and Template System (Scope Redesign)

**Status:** Design locked September 2026 (TODO item 23 — design phase). Implementation and migration pending. This section documents the scene-based architecture that will supersede the chapter-plan authoring path. Until migration (plan pending, see §16.7), chapter plans (§7, §8, §9) remain the operational path.

### 16.1 Authoring model

- **Scene** = the authoring and continuity unit. One Scene Plan per Producer call.
- **Scene files are 100% Producer-generated.** The user does not hand-author scene files or per-panel data. The user's authoring inputs (which blocks, direction, pacing) are Producer-call inputs — a UI concern designed separately, after the format.
- **Building blocks: Panel, Strip, Layout.** Chapter is *not* a system construct — it survives only as an optional `chapter_tag` on the scene file, used for filename sorting and human organization, with zero backend meaning.
- **Layout = one full page, by design.** Scenes may start or end partway down a page via loose strips/panels (edge case); the author owns placement decisions and the Producer accounts for them.

### 16.2 Templates (registry model)

Three template menu files, referenced by ID from scene files and resolved by the Parser at parse time:

- `templates/panels.yaml` — named **size presets**: width + height, nothing more.
- `templates/strips.yaml` — ordered lists of panel size refs. No strip-level geometry: strip height is **derived** (the max of its panels' heights).
- `templates/layouts.yaml` — ordered lists of strip/panel refs; one full page. Menu files may cross-reference each other.

Scene files contain no geometry. Reproducibility lives in the PanelSpec snapshot, not the file: template edits only affect *future* parses, never already-generated panels. A scene referencing a missing or renamed template fails parse — the silent-breakage guard.
...
Two top-level content sections: `elements` (structure) and `panels` (content). Example:

```yaml
# scenes/c01_s702.yaml — Producer-generated, Parser-consumed
scene_id: s702
chapter_tag: c01              # optional; filename sorting only
title: "Alyssa buys a soccer ball"
narrative: |                  # scene continuity storyboard (Producer-written)
  Alyssa leaves her apartment and heads to the market...

elements:                     # ordered; flows top-down from scene top
  - layout: layout_02         # full page; resolved from templates/layouts.yaml
  - layout: layout_01
  - strip: eq3                # loose strip — edge case, starts at top-left
  - panel: inset_square       # bare panel — treated as a 1-panel strip

panels:                       # keyed by derived panel ID; exact coverage required
  s702_l01_st01_pn01:
    description: "Wide establishing shot of the market street at midmorning."
    characters:
      - id: alyssa
        costume: morning_routine
    environment: city_exterior
...
The header set is `scene_id`, optional `chapter_tag`, `title`, `narrative`, `elements` — plus the `panels` map, that is the entire file. `narrative` is the Producer-written continuity storyboard that feeds the Scene Prompt Generator's context. No hints, no page placement, no geometry exist in the file.

### 16.5 Determinism and identity

- The Parser derives the panel set from `elements` deterministically: element index → `l`, strip index within element → `st`, position within strip → `pn`. A bare panel is a one-panel strip; floating panels count as strip members.
- **Panel ID:** `s{scene}_l{layout}_st{strip}_pn{position}`, with optional `cXX` chapter-tag prefix on filenames for sorting only.
...
### 16.7 Additive-fields principle

Panel entries and the scene header accept optional additive fields. A future field (`body_language`, `transition: {type: ...}`, pacing — per TODO items 13/14/19) means a schema version bump and a Parser change to carry the field into the PanelSpec snapshot — zero structural redesign. Known limit: panel-to-panel *references by ID* would fight positional identity (§16.5); if ever needed, that is a deliberate reopening, not an accident.
...
- **Character accountability in the stage-2 prompt:** respect character exclusions alongside environment/style exclusions. For each panel, the description and `characters[]` must agree in both directions: every named or depicted character is rostered, and every rostered character is described as present. This is a prompt-level creative constraint, not a deterministic prose-to-roster validation gate; review generated plans for misses.
...
- **Retry lives at the element level** (bounded, per-call): enforce the expected array length, panel schema, and roster/costume/environment IDs. Character mentions in free-form prose and exclusions remain prompt-level constraints, not deterministic validation gates.
...
**Addendum (scene-scoped generation, pre-cutover):** `generate --scene <id>` re-parses the committed scene via ScenePlanParser (deterministic: byte-identical PanelSpecs, stable seeds) and drives the shared Orchestrator panel-by-panel in reading order — continue-on-failure, sequential. Context windows are element-grouped (the Phase 3 work item above, landed early): same-element neighbors as Previous/Next, plus a cross-element Previous handoff at element starts (previous element's final panel) mirroring the producer's WHERE-WE-LEFT-OFF. No cross-element Next. `--panel` combines with `--scene` for single-panel scope. Chapter path (`--chapter`) is untouched; the `pipeline_mode` flag flip and CLI-defaults switch remain the true cutover.

**Addendum 2 (display-stem naming + scene regeneration):** Output PNGs now use a display stem — `chapter_tag` prefix + tag-free panel_id (`c01_s702_l01_st01_pn01_attempt_001.png`), completing the provenance-key decision. Chapter-mode PanelSpecs carry no `chapter_tag` (the tag lives inside their panel_id), so their filenames are unchanged. Provenance identity (record keys, `output/{panel_id}.provenance.jsonl`, panelspec filenames) remains tag-free; only the PNG name and the provenance `outcome.output_file` value carry the prefix. `regenerate --scene <id>` sources its PanelSpec via ScenePlanParser with element-grouped context (same helper as `generate --scene`); the four-category machinery below the CLI is unchanged. Pre-existing scene outputs generated before this change are migrated by `scripts/migrate_scene_output.py` (dry-run default; renames PNGs in output/ and output/archive/, patches path refs inside provenance records; never touches identity files).

**Addendum 3 (exclusion-cue convention, shared with the chapter producer):** Stage-2 roster context now uses the shared deterministic builder `src/roster_context.py` (see §7 exclusion-cue convention): `rarely seen` physical-description fields are suppressed as writing cues, and character exclusions are promoted to `IDENTITY RULE` lines. This is the first layer of the item-29 exclusion work; the second layer (a pre-commit gate checking returned panel prose against rostered characters' exclusions) remains open. Environment exclusions keep their existing footnote format.
